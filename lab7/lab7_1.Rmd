...
n_distinct() is a very handy way of cleanly presenting the number of distinct observations. Here we show the number of distinct genera over 100 in body weight.
```{r}
n_distinct(filter(msleep,bodywt > 100))
```
...
msleep%>%
  group_by(order)%>%
  summarize(mean_brwt_order=mean(brainwt),
            total=n())
```
...
```
NA means the data is missing. If the group contains NA, the summarize function would directly makes the output as NA.

3. Try running the code again, but this time add `na.rm=TRUE`. What is the problem with Cetacea? Compare this to Carnivora. 
```{r}
msleep%>%
  group_by(order)%>%
  summarize(mean_brwt_order=mean(brainwt,na.rm = TRUE),
            total=n())
```
All 3 Cetacea do not have data about brain weight, causing the result show "NaN", while Canivora having result though containing NA data.
