...
```{r}
any(is.na.data.frame(mammals))
```

```{r}
average_maxlife<-mean(mammals$`max. life`)
```
...
names(mammals)
```
The names of the columns are: "order","family","Genus","species","mass",      "gestation","newborn","weaning","wean mass","AFR","max. life","litter size","litters/year"

4. Rename any columns that have capitol letters or punctuation issues.  
```{r}
names(mammals)[names(mammals)=="Genus"]<-"genus"
names(mammals)[names(mammals)=="AFR"]<-"afr"
names(mammals)[names(mammals)=="max.life"]<-"maxlife"
...
```{r}
exclude1<-select(mammals,-order,-family)
```

7. Select the columns that include "mass" as part of the name.  
```{r}
massinclude<-select(mammals, contains("mass"))
```

8. Select all of the columns that are of class `character`.  
```{r}
classchara<-select_if(mammals,is.character)
```
