...
```{r}
meanspeed<-sum(venom_GT, chiron, veyron)/3
meanspeed
```
...
NA means unknown. Having NA as result means that there are unknown values causing the result to be unable to know.
...
```
The code removes the NA values in the vector and able to run calculations directly. It is useful since in many cases the data may contain empty values
